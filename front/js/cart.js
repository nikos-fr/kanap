...
order.addEventListener("click", (event) => {
  ...
  const firstName = /^[A-Z][A-Za-z\é\è\ê\ï\ë\-]+$/;
  const lastName = /^[A-Z][A-Za-z\é\è\ê\ï\ë\-]+$/;
  const address = /^[a-zA-Z0-9àèìòùÀÈÌÒÙÁÉÍÓÚÝâêîôûÂÊÎÔÛëïçÇ\s,'-]*$/;
  const city = /^[a-zA-Z]+(?:[\s-][a-zA-Z]+)*$/;
  const email = /^[A-z0-9._-]+[@]{1}[a-zA-Z0-9._-]+[.]{1}[a-zA-Z]{2,10}$/;
  // test de l'expression régulière avec condition que les champs soient remplis correctement et message d'alerte
  const messError1 = document.querySelector("#firstNameErrorMsg");
  if (firstName.test(prenom) && prenom != "") {
  ...
  const messError2 = document.querySelector("#lastNameErrorMsg");
  if (lastName.test(nom) && nom != "") {
  ...
  const messError3 = document.querySelector("#addressErrorMsg");
  if (address.test(adresse) && adresse != "") {
  ...
  const messError4 = document.querySelector("#cityErrorMsg");
  if (city.test(ville) && ville != "") {
  ...
  const messError = document.querySelector("#emailErrorMsg");
  if (email.test(mail) && email != "") {
    messError.innerText = "";
  } else {
